...
class MFF(nn.Module):# Hierarchical deep feature aggregation module
    def __init__(self):
        super(MFF, self).__init__()

        self.conv9 = torch.nn.Conv1d(in_channels=64, out_channels=64, kernel_size=9,padding =4)
        self.conv7 = torch.nn.Conv1d(in_channels=64, out_channels=64, kernel_size=7,padding =3)
        self.conv5 = torch.nn.Conv1d(in_channels=64, out_channels=64, kernel_size=5,padding =2)
        self.conv3 = torch.nn.Conv1d(in_channels=64, out_channels=64, kernel_size=3,padding =1)
        self.conv8 = torch.nn.Conv1d(in_channels=256, out_channels=64, kernel_size=3,padding =1)
        self.conv6 = torch.nn.Conv1d(in_channels=192, out_channels=64, kernel_size=3,padding =1)
        self.conv4 = torch.nn.Conv1d(in_channels=128, out_channels=64, kernel_size=3,padding =1)
...
class AHA(nn.Module): # Aggregate token awareness moudle
    def __init__(self):
        super(AHA, self).__init__()

    def forward(self, x):
        R_avg = torch.mean(x, dim=1, keepdim=True)
        x = x -R_avg
        return  x
